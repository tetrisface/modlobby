...
/**
 * Every section, in the order the page shows them; the index beside the page
 * is drawn from this. Most-reached-for first, the switches behind the
 * defaults last.
 */
const SECTIONS = {
  servers: 'Servers',
...
/** Where the page opens, and what the index marks until you scroll. */
 ...
 * for the index to call that section the one being read.
 */
const READ_LINE = 48
...
/** One titled part of the page, and a place the index can jump to. */
function Section(props: ParentProps<{ id: SectionId }>) {
  return (
    <Heading.Provider value={SECTIONS[props.id]}>
      <fieldset id={anchor(props.id)} class='set-section'>
        <legend>{SECTIONS[props.id]}</legend>
        {props.children}
      </fieldset>
    </Heading.Provider>
  )
...
export function SettingsView() {
  ...
  /**
   * Set while the page scrolls to where the index sent it. That scroll is not
   * the reader's, and near the foot of the page it stops short of putting the
   * section at the top, so it would otherwise mark a neighbour instead of the
   * entry that was clicked. The reader's own wheel takes over at once.
   */
  let jumping = false
      ...
      <div class='settings-page'>
        <nav class='settings-index' aria-label='Settings sections'>
          <SearchBox
            ...
            onInput={setQuery}
          />
          {/* While searching, the page itself is the list of what matched. */}
          <Show when={!searching()}>
            <For each={SECTION_IDS}>
              {(id) => (
                <button
                  type='button'
                  classList={{ on: reading() === id }}
        ...
        <Query.Provider value={query}>
          <div class='settings-body'>
            ...
            <p class='muted'>
              Stored as JSONC you can edit by hand; the app reloads it live and
              keeps your comments.{' '}
            ...
            <Section id='account'>
              <Row>
                ...
                <p class='muted'>
                  ...
                  password, and these decide whether any of them are kept and
                  used at startup. A password never goes in the settings file.
            ...
            <Section id='notifications'>
              <Row>
                ...
                <p class='muted'>
                  Silences every row below without changing any of them, for
                  when the lobby is open beside something that matters more. The
                  corner still shows what modlobby itself has to say — an error,
                  a download — since that is an answer to something you did.
                </p>
              </Row>
              <Row>
                <p class='muted'>
                  <b>In lobby</b> puts a line in the corner of this window.{' '}
                  <b>Desktop</b> raises a notification from your operating
                  system and flashes modlobby in the taskbar, while it is in the
            ...
            <Section id='playing'>
              ...
              <Row>
                ...
                <p class='muted'>
                  When your room's game starts, the engine starts with it —
                  while spectating too, which is otherwise the case that means
                  watching the room and pressing a button. Never without the
                  content on disk.
                </p>
              </Row>
              <Row>
                ...
                <p class='muted'>
                  Asks the pve.bar stats service — the one the in-game widget
                  uses — for a challenge score and win chance, and lists the
                  room among the games being played. Sends the map, the settings
                  and the team size; never a name or an account. Off hides the
                  panel and sends nothing.
                </p>
              </Row>
            </Section>
            ...
            <Section id='chat'>
              <Row>
                ...
                <p class='muted'>
                  ...
                  turns into the room you see. Off, those lines are shown as
                  they arrive.
                </p>
              </Row>
              <Row>
            ...
            <Section id='interface'>
              <Row>
                ...
                <p class='muted'>
                  ...
                  on its own, so plugging in a monitor does not resize the
                  laptop.
                </p>
            ...
            <Section id='overlay'>
              <Row>
                ...
                <p class='muted'>
                  Held only while a game runs, so the lobby never owns a key
                  while it sits idle. While held it does beat the game, so a
                  combination BAR uses would be taken away from it. The default
                  was picked on that basis: BAR binds <code>L</code> plain and
                  with Shift, and binds only two Alt+Shift combinations in the
                  whole game, neither of them this one.
                </p>
                <p class='muted'>
                  ...
                  by modlobby, and not by the game it starts — so Chobby finds
              ...
              <Row>
                ...
                <p class='muted'>
                  ...
                  It draws nothing, it comes back out when modlobby closes, and
                  it only takes the key when modlobby answers — so a game you
                  start from Chobby keeps its own Escape. Escape with units
                  selected still deselects them, as always.
                </p>
              </Row>
            </Section>
            ...
            <Section id='updates'>
              <Row>
                ...
                <p class='muted'>
                  ...
                  newer version puts a button beside the version in the nav.
                  With downloading on it is fetched in the background and kept,
                  nothing is installed, and the button restarts into it; the
                  next start installs it if you do not. Off, nothing is fetched
                  until you click, and the click fetches first. In a room or a
                  game the install waits for a second click. After a session
                  that ended badly the look comes round more often for a while —
                  hourly at first, easing back to daily — so a fix reaches you
                  sooner; nothing about the failure is sent anywhere.{' '}
                  <button
                    type='button'
                    disabled={updating()}
            ...
            <Section id='connection'>
              <Row>
                ...
                <p class='muted'>
                  ...
                  you are here and wrong for a window you forgot: it keeps a
                  seat in a room for nobody. Past this the connection is dropped
                  and stays dropped; the window stays, one click from logging in
            ...
            <Section id='files'>
              ...
              <Row>
                <p class='muted'>
                  ...
                  written is the <code>logging.filter</code> in the settings
                  file.
                </p>
